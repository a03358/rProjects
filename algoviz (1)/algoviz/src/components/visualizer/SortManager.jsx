...
export const SortManager = React.memo(function ({
  ...
  useEffect(() => {
    const algorithmsTimeComplexities = useControls.getState().algorithmsTimeComplexities
    progress.current = useControls.getState().progress;
      for (let index = 0; index < algorithmsTimeComplexities.length; index++) {
        const element = algorithmsTimeComplexities[index];
        if(sortingAlgorithmName === element.algorithmName)
          setCurrentAlgo(element)
          // return element.algorithmName
          // console.log("element", element.algorithmName)
          
      }
    // if()
    console.log(algorithmsTimeComplexities)
